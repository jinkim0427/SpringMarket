package inhatc.capstone.market.findMarket;

public class FindMarketVO {
	private int mk_number;
	private String mk_name;
	private String mk_address;
	private String mk_tel;
	private String mk_open_time;
	private String mk_close_time;
	private String mk_closed;
	private String mk_delivery;
	private String mk_intro;
	
	public int getMk_number() {
		return mk_number;
	}
	public void setMk_number(int mk_number) {
		this.mk_number = mk_number;
	}
	public String getMk_name() {
		return mk_name;
	}
	public void setMk_name(String mk_name) {
		this.mk_name = mk_name;
	}
	public String getMk_address() {
		return mk_address;
	}
	public void setMk_address(String mk_address) {
		this.mk_address = mk_address;
	}
	public String getMk_tel() {
		return mk_tel;
	}
	public void setMk_tel(String mk_tel) {
		this.mk_tel = mk_tel;
	}
	public String getMk_open_time() {
		return mk_open_time;
	}
	public void setMk_open_time(String mk_open_time) {
		this.mk_open_time = mk_open_time;
	}
	public String getMk_close_time() {
		return mk_close_time;
	}
	public void setMk_close_time(String mk_close_time) {
		this.mk_close_time = mk_close_time;
	}
	public String getMk_closed() {
		return mk_closed;
	}
	public void setMk_closed(String mk_closed) {
		this.mk_closed = mk_closed;
	}
	public String getMk_delivery() {
		return mk_delivery;
	}
	public void setMk_delivery(String mk_delivery) {
		this.mk_delivery = mk_delivery;
	}
	public String getMk_intro() {
		return mk_intro;
	}
	public void setMk_intro(String mk_intro) {
		this.mk_intro = mk_intro;
	}
}
